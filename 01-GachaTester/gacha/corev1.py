...
def read_config(gacha_dir):
    ...
    return pool,tier_ids,levels,base_weight,weight,tier1,tier2

def input_draw_count():
    num=int(input("输入抽取次数:"))
    #直接使用input会输入str类型，用int()控制num变量类型
    return num

def write_draw_record(results_path,result):
    r=open(results_path,'a',encoding="utf-8")
    #打开结果记录文件
    for rec in result:
        r.write("[{}]|[counter_lite: {};counter_medium: {}]|[tier: {}]|[character: {}]\n".format(
            rec["time"],rec["counter_lite"],rec["counter_medium"],rec["tier"],rec["character"]))
        #按日志格式写入单次抽卡记录
    r.close()
    #关闭文件，数据写入

def do_gacha(pool,tier_ids,levels,base_weight,weight,tier1,tier2,results_path,num):
    history=[]
...
def main():
    pool,tier_ids,levels,base_weight,weight,tier1,tier2=read_config(gacha_dir)
    results_path=gacha_dir/"results.txt"
    num=input_draw_count()
    result=do_gacha(pool,tier_ids,levels,base_weight,weight,tier1,tier2,results_path,num)
    write_draw_record(results_path,result)
    print([rec["character"] for rec in result])
